...
- `properties` (id, organization_id, portfolio_id, owner_id, property_manager_id, name, code, address_line1, address_line2, city, state, postal_code, country, latitude, longitude, property_type, status, ownership_entity, tax_id, parcel_number, year_built, total_sqft, total_units, created_at, updated_at, deleted_at)
...
- `units` (id, organization_id, property_id, building_id, floor_id, unit_number, unit_type, residential_type, bedrooms, bathrooms, half_baths, sqft, rentable_sqft, status, floor_number, market_rent_amount, current_rent_amount, created_at, updated_at, deleted_at)
...
- `tenants` (id, organization_id, full_name, email, phone, phone_secondary, type, status, date_of_birth, ssn, tax_id, current_address, emergency_contact_name, emergency_contact_phone, communication_preferences, created_at, updated_at, deleted_at)
...
## 22. Entity Media Management [MVP]

### Functional Requirements

- Highly customizable image and media management with user-defined ordering
- Support for images, videos, and other media types
- Flexible display ordering (drag-and-drop reordering)
- Automatic thumbnail selection (first image in order) or explicit thumbnail designation
- Gallery/collection support for organizing media into groups (e.g., "exterior", "interior", "before", "after")
- Add, remove, and reorder media for any entity (properties, units, leases, tenants, work orders)
- Media stored in Google Cloud Storage (GCS) with references in database
- Support for linking to existing documents table or standalone storage
- Public/private access control per media item
- Metadata storage for dimensions, tags, EXIF data, etc.
- Accessibility support with alt text

### Database Tables

- `entity_media` (id, organization_id, entity_type, entity_id, document_id, file_name, file_path, file_url, file_size, mime_type, media_type, title, description, alt_text, display_order, is_thumbnail, is_primary, gallery_name, gallery_order, is_public, is_active, metadata, uploaded_by, created_at, updated_at, created_by, updated_by, deleted_at)

### API Endpoints

- `GET /api/entities/{entity_type}/{entity_id}/media` (list all media for entity, ordered by display_order)
- `GET /api/entities/{entity_type}/{entity_id}/media?gallery={gallery_name}` (list media by gallery)
- `GET /api/entities/{entity_type}/{entity_id}/thumbnail` (get thumbnail image)
- `POST /api/entities/{entity_type}/{entity_id}/media` (add new media)
- `PUT /api/entities/{entity_type}/{entity_id}/media/reorder` (reorder media - body: { mediaIds: [5, 2, 8, 1] })
- `PUT /api/entities/{entity_type}/{entity_id}/media/{media_id}/set-thumbnail` (set as thumbnail)
- `PUT /api/entities/{entity_type}/{entity_id}/media/{media_id}` (update media metadata)
- `DELETE /api/entities/{entity_type}/{entity_id}/media/{media_id}` (soft delete media)

### Integration Points

- Document Management module (optional link via document_id)
- Property Management module (for property images)
- Unit Management module (for unit images)
- Work Order Management module (for work documentation photos)

### Dependencies

- Document Management module (optional)
