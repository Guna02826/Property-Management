...
- **Strategy**: Row-level multi-tenancy with `organization_id` on all organization-scoped tables
- **Isolation**: Database-level row-level security (RLS) policies on critical tables
- **Enforcement**: Middleware/hooks in Fastify services ensuring organization_id is set and validated on all queries
- **Super Admin**: Separate admin schema/organization for platform operations

**Implementation**:
- All tenant data tables include `organization_id` column
- Sequelize hooks and scopes automatically filter queries by organization_id from JWT claims
- Database RLS policies as defense-in-depth for critical financial tables
- Organization context injected via Fastify hooks/decorators
...
### Audit Logging
- **Table**: `audit_logs` with columns: `id`, `organization_id`, `user_id`, `action`, `entity_type`, `entity_id`, `changes`, `before_state`, `after_state`, `ip_address`, `user_agent`, `created_at`
...
- **Audit queries**: `(organization_id, entity_type, created_at DESC)` on `audit_logs`
...
- [ ] Implement multi-tenant hooks/middleware in Fastify (organization_id injection from JWT)
